...
def insertData():
    ...
    try :
        ...
        # 쿼리 실행이 완료 (오류없이)
        # 1) 메시지 박스로 성공 알림
        messagebox.showinfo("성공", "이번주도 수고했습니다.!")
        # 2) 데이터 커밋 (진짜 저장)
        conn.commit()
        # 3) 테이블 조회 (새로고침)
        selectData()
...
def selectData():
    ...
    sql = 'SELECT * FROM cashcalculator'
    cur.execute(sql)
    ...
    while(True) :
        ...
        #lgame, lgmonth, lgweek, lgold, lcashratio, lgtime
        lgame.append(row[0])
...
label5 = Label(editFrame, text="골드 현금 비율")
...
# listgmae, listgmonth, listgweek, listgold, listcasgratio, listgtime, listpay
listgmae = Listbox(listFrame)
listgmae.pack(side=LEFT, fill=BOTH, expand=1)
...
listcasgratio = Listbox(listFrame)
listcasgratio.pack(side=LEFT, fill=BOTH, expand=1)
